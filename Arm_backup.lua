...
function spinboxChange(ui, id, newValue) --??spinbox???
  ...
  elseif(id == 42) then
	...
	for i = 1, jointNum, 1 do
		simExtCustomUI_setSpinboxValue(ui, i, actionTable[currentActionIndex][i]) --??spinbox???
		simExtCustomUI_setSliderValue(ui, i + 20, actionTable[currentActionIndex][i]) --??slider???
	end
...
function actionOperate(ui,id)
  if(id == 120) then --confirm operation
    ...
    if notChange == false then
      ...
      actionTable[currentActionIndex][jointNum+1] = ResidenceTime --??????
  ...
  elseif(id == 124) then --undo operation
    ...
    for i = 1, jointNum, 1 do
      simExtCustomUI_setSpinboxValue(ui, i, actionHistory[actionHistoryIndex][i]) --??spinbox???
      simExtCustomUI_setSliderValue(ui, i + 20, actionHistory[actionHistoryIndex][i]) --??slider???
    end
